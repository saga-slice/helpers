...
function* makeCall(instance, method, path, ...args) {
    ...
    assert(path.constructor === String, 'valid path required');
    ...
    assert(success.constructor === Function, 'success must be a function');
    assert(fail.constructor === Function, 'fail must be a function');

    // validate not exist or is a function
    assert(!done || done.constructor === Function, 'done must be a function');
