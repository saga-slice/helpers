...
function* makeCall(instance, method, path, ...args) {
    ...
    affirm(method.constructor === String, 'valid method required');
    affirm(!!path && path.constructor === String, 'valid path required');
    ...
    affirm(!!success && isFunction(success), 'success must be a function');
    affirm(!!fail && isFunction(fail), 'fail must be a function');

    // validate not exist or is a function
    affirm(!done || isFunction(done), 'done must be a function');
