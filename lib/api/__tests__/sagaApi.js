...
const describeMethod = (method) => {
    ...
    describe(method, () => {
        ...
        test('should dispatch a sagaApi status action if http error', async () => {

            const args = prepareArgs();

            const error = Error('fail');
            const http = { status: 401, message: 'unauthorized' };
            ...
            expect(stub.recordAction).toHaveBeenCalledTimes(2);

            const [httpError] = stub.recordAction.mock.calls.pop();

            expect(httpError.type).toBe('sagaApi/401');
            expect(httpError.payload).toEqual(http);
...
describe('saga API', () => {
    ...
    ['get', 'post', 'put', 'patch', 'delete'].forEach(describeMethod);
